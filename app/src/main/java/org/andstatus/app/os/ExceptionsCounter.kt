 ...
 */
package org.andstatus.app.osimportimport
...
android.app.Dialogimport androidx.annotation .MainThread
import java.util.concurrent.atomic.AtomicLongimport

java.util.concurrent.atomic.AtomicReference eu.bolt.screenshotty.ScreenshotManagerBuilder.build
...
object ExceptionsCounter {
    ...
    @Volatile
    private val diskIoDialog: Dialog? = null
    val firstError: AtomicReference<String?>? = AtomicReference()
    fun getDiskIoExceptionsCount(): Long {
        return ExceptionsCounter.diskIoExceptionsCount.get()
    }

    fun onDiskIoException(e: Throwable?) {
        ExceptionsCounter.diskIoExceptionsCount.incrementAndGet()
        ExceptionsCounter.logSystemInfo(e)
    }

    fun logSystemInfo(throwable: Throwable?) {
        val systemInfo: String = MyContextHolder.Companion.myContextHolder.getSystemInfo(MyContextHolder.Companion.myContextHolder.getNow().context(), true)
    ...
    private fun logError(msgLog: String?, tr: Throwable?) {
        ...
        if (!StringUtil.isEmpty(ExceptionsCounter.firstError.get()) || tr == null) {
            return
        }
    ...
    fun showErrorDialogIfErrorsPresent() {
        if (ExceptionsCounter.diskIoExceptionsCountShown.get() == ExceptionsCounter.diskIoExceptionsCount.get()) return
        ExceptionsCounter.diskIoExceptionsCountShown.set(ExceptionsCounter.diskIoExceptionsCount.get())
        DialogFactory.dismissSafely(ExceptionsCounter.diskIoDialog)
        val text: String = StringUtil.format(MyContextHolder.Companion.myContextHolder.getNow().context(), R.string.database_disk_io_error,
                ExceptionsCounter.diskIoExceptionsCount.get())
        ExceptionsCounter.diskIoDialog = DialogFactory.showOkAlertDialog(ExceptionsCounter::class.java, MyContextHolder.Companion.myContextHolder.getNow().context(),

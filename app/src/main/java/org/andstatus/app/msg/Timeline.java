...
public class Timeline implements Comparable<Timeline> {
    ...
    public void toContentValues(ContentValues values) {
        values.put(TimelineTable._ID, id);
    ...
    public static Timeline fromCursor(Cursor cursor) {
        ...
        timeline.origin = MyContextHolder.get().persistentOrigins()
                .fromId(DbUtils.getLong(cursor, TimelineTable.ORIGIN_ID));
        timeline.account = MyContextHolder.get().persistentAccounts()

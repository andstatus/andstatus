...
class ParsedUri private constructor(intent: Intent?) {
    private val uri: Uri?
    private val matchedUri: MatchedUri?
    ...
    fun getAccountActorId(): Long {
        ...
        try {
            when (matchedUri) {
                MatchedUri.TIMELINE, MatchedUri.TIMELINE_SEARCH, MatchedUri.TIMELINE_ITEM, MatchedUri.NOTE_ITEM, MatchedUri.ORIGIN_ITEM, MatchedUri.ACTORS, MatchedUri.ACTORS_SEARCH, MatchedUri.ACTOR_ITEM -> accountActorId = uri.getPathSegments()[1].toLong()
                else -> {
                }
            }
        } catch (e: Exception) {
    ...
    fun getTimelineType(): TimelineType? {
        try {
            when (matchedUri) {
                MatchedUri.TIMELINE, MatchedUri.TIMELINE_SEARCH, MatchedUri.TIMELINE_ITEM -> return TimelineType.Companion.load(uri.getPathSegments()[3])
                else -> {
                }
            }
        } catch (e: Exception) {
    ...
    fun getActorsScreenType(): ActorsScreenType? {
        try {
            when (matchedUri) {
                MatchedUri.ACTORS, MatchedUri.ACTORS_ITEM, MatchedUri.ACTORS_SEARCH -> return ActorsScreenType.Companion.load(uri.getPathSegments()[3])
                else -> {
                }
            }
        } catch (e: Exception) {
    ...
    companion object {
        fun fromUri(uri: Uri?): ParsedUri? {
            return ParsedUri.Companion.fromIntent(Intent(Intent.ACTION_DEFAULT, uri))
        }
    ...
    init {
        uri = if (intent == null || intent.data == null) Uri.EMPTY else intent.data
        searchQuery = if (intent == null) "" else intent.getStringExtra(IntentExtra.SEARCH_QUERY.key)
        matchedUri = MatchedUri.Companion.fromUri(uri)
    }
}
...
class Convert42 extends ConvertOneStep {
    Convert42() {
        versionTo = 44;
    }
    ...
    private static class Data {
        ...
        static Data fromCursor(Cursor cursor) {
            return new Data(
                    DbUtils.getLong(cursor, "_id"),
                    Uri.parse(DbUtils.getString(cursor, "banner_url")));
        }

        private Data(long id, @NonNull Uri uri) {
            this.id = id;
            this.uri = uri;
        }
    }
    ...
    protected void execute2() {
        ...
        sql = "CREATE TABLE actorendpoints (actor_id INTEGER NOT NULL,endpoint_type INTEGER NOT NULL,endpoint_index INTEGER NOT NULL DEFAULT 0,endpoint_uri INTEGER NOT NULL, CONSTRAINT pk_actorendpoints PRIMARY KEY (actor_id ASC, endpoint_type ASC, endpoint_index ASC))";

        progressLogger.logProgress(stepTitle + ": Adding actorendpoints");

        sql = "CREATE TABLE actorendpoints (actor_id INTEGER NOT NULL,endpoint_type INTEGER NOT NULL,endpoint_index INTEGER NOT NULL DEFAULT 0,endpoint_uri INTEGER NOT NULL, CONSTRAINT pk_actorendpoints PRIMARY KEY (actor_id ASC, endpoint_type ASC, endpoint_index ASC))";
        DbUtils.execSQL(db, sql);

        sql ="SELECT _id, banner_url FROM actor WHERE banner_url NOT NULL";
        MyQuery.<Set<Data>>foldLeft(db, sql, new HashSet<>(), set -> cursor -> {
            set.add(Data.fromCursor(cursor));
            return set;
        }).stream().filter(data -> data.uri != Uri.EMPTY).forEach( data ->
            DbUtils.execSQL(db, "INSERT INTO actorendpoints (actor_id, endpoint_type, endpoint_index, endpoint_uri)" +
                    " VALUES(" + data.id + ", 7, 0, '" + data.uri + "')" // 7 - BANNER
        ));
    }
}

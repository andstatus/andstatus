...
import org.andstatus.app.data.converter.Convert40
...
internal class Convert40 : ConvertOneStep() {
    private class Data private constructor(val id: Long, val uri: String, val contentType: MyContentType) {
        companion object {
            fun fromCursor(cursor: Cursor?): Convert40.Data? {
                val uri = DbUtils.getString(cursor, "url")
                return Convert40.Data(
    ...
    protected override fun execute2() {
        ...
        MyQuery.foldLeft(db, sql, HashSet(), { set: MutableSet<Convert40.Data?>? ->
            Function { cursor: Cursor? ->
                set.add(Convert40.Data.Companion.fromCursor(cursor))
                set
            }
        }).forEach(Consumer { data: Convert40.Data? ->
            DbUtils.execSQL(db,

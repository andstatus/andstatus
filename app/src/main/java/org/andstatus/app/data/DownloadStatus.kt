...
enum class DownloadStatus(private val code: Long, private val titleResourceId: Int, canBeDownloaded: CanBeDownloaded?) {
    LOADED(2, 0, CanBeDownloaded.YES), SOFT_ERROR(4, 0, CanBeDownloaded.NO), HARD_ERROR(5, 0, CanBeDownloaded.NO), ABSENT(6, 0, CanBeDownloaded.NO), SENDING(7, R.string.download_status_unsent, CanBeDownloaded.YES), SENT(11, R.string.sent, CanBeDownloaded.YES), DRAFT(8, R.string.download_status_draft, CanBeDownloaded.NO), DELETED(9, 0, CanBeDownloaded.NO), NEEDS_UPDATE(10, 0, CanBeDownloaded.YES), UNKNOWN(0, 0, CanBeDownloaded.YES);
    ...
    fun mayBeSent(): Boolean {
        return DownloadStatus.Companion.mayBeSent(this)
    }

    fun isUnsentDraft(): Boolean {
        return when (this) {
            DownloadStatus.SENDING, DownloadStatus.DRAFT -> true
            else -> false
        }
    }
    ...
    fun mayUpdateContent(): Boolean {
        return when (this) {
            DownloadStatus.SENDING, DownloadStatus.DRAFT, DownloadStatus.LOADED -> true
            else -> false
        }
    }
    ...
    fun isPresentAtServer(): Boolean {
        return when (this) {
            DownloadStatus.SENT, DownloadStatus.LOADED -> true
            else -> false
        }
    }
    ...
    companion object {
        ...
        fun load(codeIn: Long): DownloadStatus? {
            for (`val` in DownloadStatus.values()) {
                if (`val`.code == codeIn) {
                    return `val`
                }
            }
        ...
        private fun mayBeSent(status: DownloadStatus?): Boolean {
            return when (status) {
                DownloadStatus.SENDING, DownloadStatus.HARD_ERROR, DownloadStatus.SOFT_ERROR -> true
                else -> false
            }
        }
    }

...
public class DemoMessageInserter {
    ...
    public DemoMessageInserter(Actor accountActor) {
    ...
    public Actor buildActorFromOidAndAvatar(String userOid, String avatarUrlString) {
        Actor actor = buildActorFromOid(userOid);
    ...
    final Actor buildActorFromOid(String actorOid) {
        ...
        if (origin.getOriginType() == OriginType.PUMPIO) {
            ...
            username = connection.userOidToUsername(actorOid);
    ...
    static void onActivityS(AActivity activity) {
        new DemoMessageInserter(activity.accountActor).onActivity(activity);
    }
    ...
    public void onActivity(final AActivity activity) {
        MyAccount ma = MyContextHolder.get().persistentAccounts().fromUserId(accountActor.userId);
    ...
    private void checkActivityRecursively(AActivity activity, int level) {
        ...
        assertNotEquals( "Account is unknown: " + activity, 0, activity.accountActor.userId);
        ...
        if (activity.getObjActor().nonEmpty()) {
            assertNotEquals( "User was not added: " + activity.getObjActor(), 0, activity.getObjActor().userId);
        }
    ...
    public static AActivity addMessageForAccount(MyAccount ma, String body, String messageOid, DownloadStatus messageStatus) {
        ...
        DemoMessageInserter mi = new DemoMessageInserter(accountActor);

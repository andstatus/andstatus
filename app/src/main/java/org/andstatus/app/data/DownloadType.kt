...
enum class DownloadType(private val code: Long, val filePrefix: String?, val cacheName: CacheName?) {
    AVATAR(1, "av", CacheName.AVATAR), ATTACHMENT(2, "at", CacheName.ATTACHED_IMAGE), UNKNOWN(0, "un", CacheName.ATTACHED_IMAGE);
    ...
    companion object {
        ...
        fun load(strCode: String?): DownloadType? {
            try {
                return DownloadType.Companion.load(strCode.toLong())
            } catch (e: NumberFormatException) {
                MyLog.v(DownloadType.Companion.TAG, "Error converting '$strCode'", e)
            }
            return DownloadType.UNKNOWN
        }

        fun load(code: Long): DownloadType? {
            for (`val` in DownloadType.values()) {
                if (`val`.code == code) {
                    return `val`
                }
            }

...
enum class NoteContextMenuItem @JvmOverloads constructor(private val mIsAsync: Boolean = false, val appliedToUnsentNotesAlso: Boolean = false) : ContextMenuItem {
    REPLY(true, false) {
        override fun executeAsync(menu: NoteContextMenu?): NoteEditorData? {
            return NoteEditorData.Companion.newReplyTo(menu.getNoteId(), menu.getActingAccount())
                    .addMentionsToText()
    ...
    EDIT(true, false) {
        override fun executeAsync(menu: NoteContextMenu?): NoteEditorData? {
            return NoteEditorData.Companion.load(menu.getMyContext(), menu.getNoteId())
        }
    ...
    RESEND(true, false) {
        override fun executeAsync(menu: NoteContextMenu?): NoteEditorData? {
            ...
            val commandData: CommandData = CommandData.Companion.newUpdateStatus(ma, activityId, menu.getNoteId())
    ...
    COPY_TEXT(true, true) {
        override fun executeAsync(menu: NoteContextMenu?): NoteEditorData? {
            ...
            return NoteEditorData.Companion.newEmpty(menu.getActingAccount()).setContent(body, TextMediaType.HTML)
        }
    ...
    COPY_AUTHOR(true, true) {
        override fun executeAsync(menu: NoteContextMenu?): NoteEditorData? {
            val author: Actor = Actor.Companion.load(menu.getMyContext(),
                    MyQuery.noteIdToActorId(NoteTable.AUTHOR_ID, menu.getNoteId()))
            MyLog.v(this) { "noteId:" + menu.getNoteId() + " -> author:" + author }
            return NoteEditorData.Companion.newEmpty(menu.getActingAccount()).appendMentionedActorToText(author)
        }
    ...
    ACT_AS(false, true) {
        override fun executeOnUiThread(menu: NoteContextMenu?, editorData: NoteEditorData?) {
            AccountSelector.Companion.selectAccountOfOrigin(menu.getActivity(),
    ...
    SHOW_DUPLICATES(false, true) {
        override fun executeOnUiThread(menu: NoteContextMenu?, editorData: NoteEditorData?) {
            menu.getActivity().updateList(LoadableListViewParameters.Companion.collapseOneDuplicate(
                    false, menu.getViewItem().topmostId))
        }
    },
    COLLAPSE_DUPLICATES(false, true) {
        override fun executeOnUiThread(menu: NoteContextMenu?, editorData: NoteEditorData?) {
            menu.getActivity().updateList(LoadableListViewParameters.Companion.collapseOneDuplicate(
                    true, menu.getViewItem().topmostId))
        }
    },
    GET_NOTE(true, false) {
        override fun executeAsync(menu: NoteContextMenu?): NoteEditorData? {
            val status: DownloadStatus = DownloadStatus.Companion.load(
                    MyQuery.noteIdToLongColumnValue(NoteTable.NOTE_STATUS, menu.getNoteId()))
            if (status == DownloadStatus.LOADED) {
                MyProvider.Companion.update(menu.getMyContext(), NoteTable.TABLE_NAME,
        ...
        override fun executeOnUiThread(menu: NoteContextMenu?, editorData: NoteEditorData?) {
            Note.Companion.requestDownload(menu.getActingAccount(), menu.getNoteId(), true)
        }
    },
    OPEN_NOTE_LINK {
        override fun executeOnUiThread(menu: NoteContextMenu?, editorData: NoteEditorData?) {
            NoteShare.Companion.openLink(menu.getActivity(), extractUrlFromTitle(menu.getSelectedMenuItemTitle()))
        }
    ...
    fun addTo(menu: Menu?, order: Int, titleRes: Int) {
        menu.add(MyContextMenu.Companion.MENU_GROUP_NOTE, this.id, order, titleRes)
    }

    fun addTo(menu: Menu?, order: Int, title: CharSequence?) {
        menu.add(MyContextMenu.Companion.MENU_GROUP_NOTE, this.id, order, title)
    }
    ...
    fun execute(menu: NoteContextMenu?): Boolean {
        ...
        if (mIsAsync) {
            ...
            val myAccount = menu.getActingAccount().getValidOrCurrent(menu.menuContainer.activity.myContext)
    ...
    open fun executeAsync(menu: NoteContextMenu?): NoteEditorData? {
        return NoteEditorData.Companion.newEmpty(menu.getActingAccount())
    }
    ...
    fun sendActOnActorCommand(command: CommandEnum?, myAccount: MyAccount?, actor: Actor?) {
        MyServiceManager.Companion.sendManualForegroundCommand(
                CommandData.Companion.actOnActorCommand(command, myAccount, actor, actor.getUsername()))
    }

    fun sendNoteCommand(command: CommandEnum?, editorData: NoteEditorData?) {
        MyServiceManager.Companion.sendManualForegroundCommand(
                CommandData.Companion.newItemCommand(command, editorData.ma, editorData.getNoteId()))
    }

package org.andstatus.app.net.social.pumpioimport
...
internal enum class PActivityType(val activityType: ActivityType?, val code: String?) {
    DELETE(ActivityType.DELETE, "delete"), FAVORITE(ActivityType.LIKE, "favorite"), FOLLOW(ActivityType.FOLLOW, "follow"), POST(ActivityType.CREATE, "post"), SHARE(ActivityType.ANNOUNCE, "share"), STOP_FOLLOWING(ActivityType.UNDO_FOLLOW, "stop-following"), UNFAVORITE(ActivityType.UNDO_LIKE, "unfavorite"), UNKNOWN(ActivityType.EMPTY, "unknown"), UPDATE(ActivityType.UPDATE, "update");

    companion object {
        /** Returns the enum or UNKNOWN  */
        fun load(strCode: String?): PActivityType {
            if ("unfollow".equals(strCode, ignoreCase = true)) {
                return PActivityType.STOP_FOLLOWING
            }
            if ("unlike".equals(strCode, ignoreCase = true)) {
                return PActivityType.UNFAVORITE
            }
            for (value in PActivityType.values()) {
                if (value.code.equals(strCode, ignoreCase = true)) {
                    return value
                }
            }
            return PActivityType.UNKNOWN
        }
    }
}
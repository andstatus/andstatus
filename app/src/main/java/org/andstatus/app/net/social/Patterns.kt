...
java.util.regex.Pattern eu.bolt.screenshotty.ScreenshotManagerBuilder.build
...
object Patterns {
    ...
    val WEBFINGER_ID_REGEX_PATTERN = Pattern.compile(Patterns.WEBFINGER_ID_REGEX)
    private val USERNAME_REGEX_SIMPLE: String? = "^[_A-Za-z0-9-+]+(\\.[_A-Za-z0-9-]+)*$"
    val USERNAME_REGEX_SIMPLE_PATTERN = Pattern.compile(Patterns.USERNAME_REGEX_SIMPLE)
    val USERNAME_CHARS: String? = "._ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-+"
    val WEBFINGER_ID_CHARS: String? = Patterns.USERNAME_CHARS + "@"
    private val USERNAME_REGEX_ALL_CHARS: String? = "^[_A-Za-z0-9-+]+([_A-Za-z0-9-+@.]+)*$"
    val USERNAME_REGEX_ALL_CHARS_PATTERN = Pattern.compile(Patterns.USERNAME_REGEX_ALL_CHARS)
}
...
class HttpRequest private constructor(val apiRoutine: ApiRoutineEnum?, val uri: Uri?) {
    private var connectionData: HttpConnectionData? = HttpConnectionData.Companion.EMPTY
    ...
    override fun toString(): String {
        return ("uri:'" + uri + "'"
                ...
                + postParams.map(Function { params: JSONObject? -> "'$params'" }).orElse("(nothing)")
    ...
    fun withAttachmentToPost(attachment: Attachment?): HttpRequest? {
        mediaUri = Optional.ofNullable(attachment.mediaUriToPost()).filter { obj: Uri? -> UriUtils.nonEmpty() }
        return asPost()
    }
    ...
    fun getLogName(): String? {
        return if (connectionData.getOriginType() === OriginType.ACTIVITYPUB || connectionData.getOriginType() === OriginType.PUMPIO || apiRoutine == ApiRoutineEnum.OAUTH_REGISTER_CLIENT) {
    ...
    companion object {
        fun of(apiRoutine: ApiRoutineEnum?, uri: Uri?): HttpRequest? {
            return org.andstatus.app.net.http.HttpRequest(apiRoutine, uri)
        }
    }

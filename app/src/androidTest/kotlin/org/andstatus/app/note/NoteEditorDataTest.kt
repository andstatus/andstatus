...
class NoteEditorDataTest {
    ...
    fun noteEditorDataConversation() {
        ...
        val origin: Origin = myContextHolder.getNow().origins.fromName(demoData.conversationOriginName)
        ...
        val memberActorId = MyQuery.oidToId(
            OidEnum.ACTOR_OID, origin.id,
    ...
    private fun assertData(
        ma: MyAccount, inReplyToMsgId: Long, inReplyToActorId: Long, recipientId: Long,
    ...
    private fun assertMentionedActor(data: NoteEditorData, mentionedActorId: Long, isMentionedExpected: Boolean) {
        ...
        val expectedName = MyQuery.actorIdToStringColumnValue(
            if (data.ma.origin.isMentionAsWebFingerId()) ActorTable.WEBFINGER_ID else ActorTable.USERNAME,
            mentionedActorId
        )
    ...
    fun testAddMentionsWhenNobodyIsMentioned() {
        ...
        val noteId = MyQuery.getLongs(
            myAccount.myContext, "SELECT " + BaseColumns._ID
                + " FROM " + NoteTable.TABLE_NAME
                + " WHERE " + NoteTable.ORIGIN_ID + "=" + myAccount.origin.id
                + " AND " + NoteTable.CONTENT + "='Older one note'"
        ).stream()
            .findFirst().orElse(0L)

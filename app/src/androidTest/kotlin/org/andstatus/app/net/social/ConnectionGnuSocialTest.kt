...
class ConnectionGnuSocialTest {
    ...
    fun testGetPublicTimeline() {
        stub.http.addResponse(org.andstatus.app.test.R.raw.quitter_home)
        ...
        val timeline = stub.connection.getTimeline(
            true, ApiRoutineEnum.PUBLIC_TIMELINE,
        ...
        Assert.assertEquals(
            "Description",
            "Manjaro user, student of physics and metalhead. Excuse my english ( ͡° ͜ʖ ͡°)",
            author.getSummary()
        )
        ...
        Assert.assertEquals(
            "Avatar URL",
            "http://quitter.se/avatar/116387-48-20140609172839.png",
            author.getAvatarUrl()
        )
        ...
        Assert.assertEquals(
            "Created at",
            stub.connection.parseDate("Sun Feb 09 22:33:42 +0100 2014"),
        ...
        Assert.assertEquals("conversationOid", "2218650", activity.getNote().conversationOid)
        Assert.assertEquals(
            "Should have a recipient $activity",
            1,
            activity.audience().getNonSpecialActors().size.toLong()
        )
        ...
        Assert.assertEquals(
            "Updated date should be 0 for inReplyTo note", RelativeTime.DATETIME_MILLIS_NEVER,
        ...
        Assert.assertEquals(
            "Body of this note starts with",
            startsWith,
            activity.getNote().content.substring(0, startsWith.length)
        )
        ...
        UriUtilsTest.assertEndpoint(
            ...
            "https://quitter.se/file/3fd65c6088ea02dc3a5ded9798a865a8ff5425b13878da35ad894cd084d015fc.png",
            author
        )
        ind++
        activity = timeline[ind] ?: throw IllegalStateException("No activity")
        ...
        Assert.assertEquals(
            "Should be public",
            Visibility.PUBLIC_AND_TO_FOLLOWERS,
            activity.getNote().audience().visibility
        )
        Assert.assertEquals(
            "Favorited $activity",
            TriState.UNKNOWN,
        ...
        Assert.assertEquals(
            "Avatar URL",
            "http://quitter.se/avatar/114973-48-20140702161520.jpeg",
            author.getAvatarUrl()
        )
        UriUtilsTest.assertEndpoint(ActorEndpointType.BANNER, "", author)
        ...
        Assert.assertEquals(
            "Created at",
            stub.connection.parseDate("Wed Aug 14 10:05:28 +0200 2013"),
            author.getCreatedDate()
        )
        Assert.assertEquals("Updated at", 0, author.getUpdatedDate())
    }
    ...
    fun testSearch() {
        ...
        val timeline = stub.connection.searchNotes(
            true,
            TimelinePosition.Companion.EMPTY,
            TimelinePosition.Companion.EMPTY,
            20,
            DemoData.demoData.globalPublicNoteText
        ).get()
        Assert.assertNotNull("timeline returned", timeline)
        val size = 4
        Assert.assertEquals("Number of items in the Timeline", size.toLong(), timeline.size().toLong())
    ...
    fun testPostWithMedia() {
        stub.http.addResponse(org.andstatus.app.test.R.raw.quitter_note_with_attachment)
    ...
    private fun privateGetNoteWithAttachment(uniqueUid: Boolean): AActivity {
        ...
        val attachment: Attachment =
            Attachment.Companion.fromUri("https://quitter.se/file/mcscx-20131110T222250-427wlgn.png")
    ...
    fun testReblog() {
        ...
        stub.http.addResponse(org.andstatus.app.test.R.raw.loadaverage_repost_response)
        ...
        Assert.assertEquals(1, stub.http.getRequestsCounter())
    ...
    private fun getFavoritingActivity(
        favoritingOid: String?,
        favoritingContent: String?,
        likedContent: String?
    ): AActivity {
        val accountActor: Actor = DemoData.demoData.getAccountActorByOid(DemoData.demoData.gnusocialTestAccountActorOid)
        ...
        val activityIn: AActivity = AActivity.Companion.newPartialNote(
            accountActor, actor, favoritingOid, favoritingUpdateDate,
    ...
    fun testFavoritingActivityInTimeline() {
        stub.http.addResponse(org.andstatus.app.test.R.raw.loadaverage_favoriting_activity)
        val accountActor: Actor = DemoData.demoData.getAccountActorByOid(DemoData.demoData.gnusocialTestAccountActorOid)
        val timeline = stub.connection.getTimeline(
            true, ApiRoutineEnum.SEARCH_NOTES,
            TimelinePosition.Companion.of("2656388"), TimelinePosition.Companion.EMPTY, 20, accountActor
        ).get()
        Assert.assertNotNull("timeline returned", timeline)
        ...
        Assert.assertEquals(
            "Updated date should be 1 for favorited note", RelativeTime.SOME_TIME_AGO,
    ...
    fun testMentionsInHtml() {
        ...
        val activity = oneHtmlMentionsTest(
            "andstatus", "13421701",
            org.andstatus.app.test.R.raw.loadaverage_note_with_mentions3, 1
        )
        val spannable = SpanUtil.textToSpannable(
            activity.getNote().content, TextMediaType.HTML,
            activity.audience()
        )
        ...
        Assert.assertEquals(
            ...
            Arrays.stream(spans).filter { span: MyUrlSpan -> span.getURL()?.contains("/search/%23Hubzilla") == true }
                .findAny()
                .orElse(MyUrlSpan.Companion.EMPTY).data.getTimeline().timelineType
        )
    }

    private fun oneHtmlMentionsTest(
        actorUsername: String,
        noteOid: String,
        responseResourceId: Int,
        numberOfMembers: Int
    ): AActivity {
        stub.http.addResponse(responseResourceId)
        ...
        val executionContext = CommandExecutionContext(
            myContext, CommandData.Companion.newItemCommand(CommandEnum.GET_NOTE, stub.data.getMyAccount(), 123)
        )
        ...
        val storedAudience: Audience =
            Audience.Companion.load(activity.getNote().origin, activity.getNote().noteId, Optional.empty())
    ...
    private fun assertAudience(activity: AActivity, audience: Audience, numberOfMembers: Int) {
        ...
        Assert.assertEquals(
            "Wrong number of audience members $audience\n$activity",
            numberOfMembers.toLong(),
            actors.size.toLong()
        )
        Assert.assertEquals(
            "All recipients should have valid usernames $audience\n$activity", Actor.EMPTY,
            actors.stream().filter { actor: Actor -> !actor.isUsernameValid() }.findAny().orElse(Actor.EMPTY)
        )
        Assert.assertEquals(
            "All recipients should have id $audience\n$activity", Actor.EMPTY,
            actors.stream().filter { actor: Actor -> actor.actorId == 0L }.findAny().orElse(Actor.EMPTY)
        )
        Assert.assertEquals(
            "All recipients should be nonEmpty $audience\n$activity", Actor.EMPTY,
            actors.stream()

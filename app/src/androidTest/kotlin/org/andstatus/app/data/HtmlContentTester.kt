...
class HtmlContentTester {
    ...
    private fun assertHtmlNote(author: Actor, bodyString: String?, noteOid: String?) {
        if (author.origin.isHtmlContentAllowed()) {
            ...
            assertHtmlNoteContentAllowed(
                author, "$bodyString no HTML",
                if (noteOid.isNullOrEmpty()) null else "$noteOid-noHtml", false
            )
        }
    }

    private fun assertHtmlNoteContentAllowed(
        author: Actor,
        ...
        val activity = mi.buildActivity(
            author, "", bodyString, null, noteOid,
            DownloadStatus.LOADED
        )
        mi.onActivity(activity)
        val noteStored: Note = Note.Companion.loadContentById(myContextHolder.getNow(), activity.getNote().noteId)
        ...
        if (htmlContentAllowed) {
            ...
            Assert.assertFalse(
                "HTML should be removed: " + noteStored.content,
                noteStored.content.replace(MyHtml.LINEBREAK_HTML.toRegex(), "\n").contains("<")
            )
        }
        Assert.assertEquals(
            "Stored content " + activity.getNote(), activity.getNote().getContentToSearch(),
    ...
    init {
        val origin: Origin = myContextHolder.getNow().origins.fromName(DemoData.demoData.conversationOriginName)
        Assert.assertTrue(
            DemoData.demoData.conversationOriginName + " exists",

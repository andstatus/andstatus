...
class DataPrunerTest {
    ...
    fun testPrune() {
        ...
        clearPrunedDate()
        val dp = DataPruner(myContextHolder.getNow())
        ...
        if (logFile1.setLastModified(lastModifiedNew)) {
            ...
            Assert.assertTrue("Pruned", dp.prune())
            Assert.assertFalse(
                "File " + logFile2.name + " was old: " + millisToDateString(lastModifiedNew),
                logFile2.exists()
            )
        } else {
    ...
    fun testPruneParentlessAttachments() {
        ...
        val dd: DownloadData = DownloadData.Companion.fromAttachment(
            -555L,
    ...
    fun testPruneMedia() {
        ...
        Assert.assertNotEquals(
            "Something should be pruned, dir size: " + dirSize1

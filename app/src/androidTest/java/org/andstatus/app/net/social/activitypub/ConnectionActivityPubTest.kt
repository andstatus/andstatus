...
class ConnectionActivityPubTest {
    ...
    fun getTimeline() {
        ...
        Assert.assertEquals("Creating a Note $activity", AObjectType.NOTE, activity.objectType)
        ...
        Assert.assertEquals("Activity updated at " + TestSuite.utcTime(activity.updatedDate),
                ...
                TestSuite.utcTime(activity.updatedDate).toString())
        ...
        Assert.assertEquals("Actor's Webfinger $activity", "", actor.webFingerId)
        Assert.assertEquals("Actor is an Author", actor, activity.author)
        ...
        Assert.assertEquals("Actor", "https://pleroma.site/users/ActivityPubTester", activity.actor.oid)
        Assert.assertEquals("Actor is my friend", TriState.UNKNOWN, activity.actor.isMyFriend)
        ...
        for (ind in 0..2) {
            ...
            Assert.assertEquals("WebFinger ID", "andstatus@pleroma.site", objActor.webFingerId)
    ...
    fun getNotesByActor() {
        ...
        Assert.assertEquals("Announcing a Note $activity", AObjectType.NOTE, activity.objectType)
        val note = activity.note
    ...
    fun noteFromPawooNet() {
        ...
        Assert.assertEquals("Acting on a Note $activity8", AObjectType.NOTE, activity8.objectType)
    ...
    fun getTimeline2() {
        ...
        Assert.assertEquals("Creating a Note $activity9", AObjectType.NOTE, activity9.objectType)
        ...
        Assert.assertEquals("Activity updated at " + TestSuite.utcTime(activity9.updatedDate),
                TestSuite.utcTime(2019, Calendar.MARCH, 15, 4, 38, 48).toString(),
                TestSuite.utcTime(activity9.updatedDate).toString())
        ...
        Assert.assertEquals("Actor's Webfinger $activity9", "", actor9.webFingerId)
    ...
    fun testGetNoteWithAudience() {
        ...
        Assert.assertEquals("Acting on a Note $activity", AObjectType.NOTE, activity.objectType)
        val note = activity.note
        ...
        Assert.assertEquals("Actor is author", author, activity.actor)

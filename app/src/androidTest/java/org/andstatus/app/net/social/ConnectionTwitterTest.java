...
public class ConnectionTwitterTest extends InstrumentationTestCase {
    ...
    public void testGetTimeline() throws IOException {
        ...
        assertTrue("Is not a reblog", message.getReblogged().isEmpty());
        ...
        assertEquals("Reply to the message by userOid", TestSuite.TWITTER_TEST_ACCOUNT_USER_OID, message.getInReplyTo().sender.oid);
        ...
        message = timeline.get(ind).mbMessage;
        assertTrue("Does not have a recipient", message.recipient == null);
        assertTrue("Is a reblog", message.getReblogged().nonEmpty());
        assertTrue("Is not a reply", message.getInReplyTo().isEmpty());
        assertEquals("Reblog of the message id", "315088751183409153", message.getReblogged().oid);
        assertEquals("Reblog of the message by userOid", "442756884", message.getReblogged().sender.oid);
        assertTrue("Is not Favorited", !message.getFavoritedByActor().toBoolean(true));
        startsWith = "RT @AndStatus1: This AndStatus application";
        assertEquals("Body of this message starts with", startsWith, message.getBody().substring(0, startsWith.length()));
        startsWith = "This AndStatus application";
        assertEquals("Body of reblogged message starts with", startsWith, message.getReblogged().getBody().substring(0, startsWith.length()));
        ...
        assertEquals("Reblogged message created at Fri Mar 22 13:13:07 +0000 2013 (" + date.toString() + ")", date.getTime(), message.getReblogged().sentDate);

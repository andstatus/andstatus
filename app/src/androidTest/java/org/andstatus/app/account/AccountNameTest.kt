...
class AccountNameTest {
    ...
    fun preserveInputValues() {
        assertForOneOrigin(Origin.Companion.EMPTY)
    ...
    fun assertInputValues(origin: Origin?, uniqueName: String?) {
        ...
        val expected = if (StringUtil.isEmpty(uniqueName)) "" else if (uniqueName.contains("@")) uniqueName else uniqueName + if (origin.hasHost()) "@" + origin.getHost() else ""
        Assert.assertEquals(expected, accountName.uniqueName)
    }

    @Test
    fun testUniqueAccountName() {
        val accountName1: AccountName = AccountName.Companion.fromAccountName(MyContextHolder.Companion.myContextHolder.getNow(), "someTester/Pump.io")
        Assert.assertEquals(accountName1.toString(), accountName1.origin.name, "Pump.io")
        val accountName2: AccountName = AccountName.Companion.fromAccountName(MyContextHolder.Companion.myContextHolder.getNow(), "someTester/PumpioTest")

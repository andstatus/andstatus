...
class AccountSettingsActivityTest : ActivityTest<AccountSettingsActivity?>() {
    ...
    fun test() {
        ...
        activity.finish()
        DbUtils.waitMs(method, 500)
        FirstActivity.Companion.closeAllActivities(instrumentation.targetContext)
        DbUtils.waitMs(method, 500)
    }
    ...
    private fun assertUniqueNameTextField(viewId: Int) {
        ...
        Assert.assertEquals("Unique name of selected account $ma", ma.getOAccountName().uniqueName, uniqueNameText.text.toString())
    }

    @Throws(InterruptedException::class)
    private fun openingOriginList() {
        val method = "testOpeningOriginList"
        val activityMonitor = instrumentation.addMonitor(PersistentOriginList::class.java.name, null, false)
        ...
        val nextActivity = instrumentation.waitForMonitorWithTimeout(activityMonitor, 15000)

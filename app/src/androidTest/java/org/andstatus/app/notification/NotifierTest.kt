...
class NotifierTest {
    ...
    fun testCreateNotification() {
        ...
        notifier.clearAll()
        Assert.assertTrue("Events should be empty " + notifier.events, notifier.events.isEmpty)
        NotifierTest.Companion.addNotificationEvent(myContext, NotificationEventType.PRIVATE)
        ...
        NotifierTest.Companion.addNotificationEvent(myContext, NotificationEventType.MENTION)
        assertCount(notifier, 0, NotificationEventType.ANNOUNCE)
        assertCount(notifier, 1, NotificationEventType.MENTION)
        assertCount(notifier, 1, NotificationEventType.PRIVATE)
        NotifierTest.Companion.addNotificationEvent(myContext, NotificationEventType.ANNOUNCE)
    ...
    companion object {
        fun addNotificationEvent(myContext: MyContext?, eventType: NotificationEventType?) {
            val where = "SELECT " + ActivityTable.TABLE_NAME + "." + BaseColumns._ID +
                    ...
                    " WHERE " + ActivityTable.ACTIVITY_TYPE + "=" + NotifierTest.Companion.eventTypeToActivityType(eventType).id +

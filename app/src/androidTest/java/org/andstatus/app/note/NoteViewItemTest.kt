...
class NoteViewItemTest {
    ...
    fun testDuplicationLink() {
        ...
        NoteViewItemTest.Companion.setContent(item1, NoteViewItemTest.Companion.HTML_BODY)
        val item2 = NoteViewItem(false, RelativeTime.DATETIME_MILLIS_NEVER)
        NoteViewItemTest.Companion.setContent(item2, "Some other text")
        assertDuplicates(item1, DuplicationLink.DUPLICATES, item2)
        item2.setNoteId(2)
        assertDuplicates(item1, DuplicationLink.NONE, item2)
        NoteViewItemTest.Companion.setContent(item2, "@<a href=\"https://bsdnode.xyz/actor/2\" class=\"h-card mention\">username</a> On duplicated posts, sent by AndStatus, please read <a href=\"https://github.com/andstatus/andstatus/issues/83\" title=\"https://github.com/andstatus/andstatus/issues/83\" class=\"attachment\" rel=\"nofollow\">https://github.com/andstatus/andstatus/issues/83</a><br /> Sorry if I misunderstood your post :-)")
        assertDuplicates(item1, DuplicationLink.DUPLICATES, item2)
        NoteViewItemTest.Companion.setContent(item1, "&quot;Interactions&quot; timeline in Twidere is the same or close to existing &quot;Mentions&quot; timeline in AndStatus")
        NoteViewItemTest.Companion.setContent(item2, "\"Interactions\" timeline in Twidere is the same or close to existing \"Mentions\" timeline in AndStatus")
        assertDuplicates(item1, DuplicationLink.DUPLICATES, item2)
        val content5 = "What is good about Android is that I can use Quitter.se via AndStatus."
        NoteViewItemTest.Companion.setContent(item1, content5)
        ...
        NoteViewItemTest.Companion.setContent(item2, content6)
        assertDuplicates(item1, DuplicationLink.DUPLICATES, item2)
        assertDuplicates(item2, DuplicationLink.DUPLICATES, item1)
        val item3 = NoteViewItem(false, 1468509659000L)
        NoteViewItemTest.Companion.setContent(item3, content5)
        val item4 = NoteViewItem(false, 1468509658000L)
        NoteViewItemTest.Companion.setContent(item4, content6)
        item4.setNoteId(item2.noteId)
        ...
        NoteViewItemTest.Companion.setContent(item5, content6)
        item5.setNoteId(item4.noteId)
...
Sorry if I misunderstood your post :-)"""
        ...
        private fun setContent(item: NoteViewItem?, content: String?) {
            ...
            item.getContentToSearch() = MyHtml.getContentToSearch(content)
        }
    }
}
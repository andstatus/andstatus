...
class SqlIdsTest {
    ...
    @Test
    fun fromTimeline() {
        val myAccount: MyAccount = MyContextHolder.Companion.myContextHolder.getNow().accounts().fromAccountName(DemoData.Companion.demoData.conversationAccountName)
        Assert.assertTrue("account is not valid: " + DemoData.Companion.demoData.conversationAccountName, myAccount.isValid)
        val timeline: Timeline = MyContextHolder.Companion.myContextHolder.getNow().timelines().filter(false, TriState.FALSE,
        ...
        val timelineCombined: Timeline = MyContextHolder.Companion.myContextHolder.getNow().timelines().filter(false, TriState.TRUE,
                TimelineType.SENT, Actor.Companion.EMPTY, Origin.Companion.EMPTY).findFirst().orElse(Timeline.Companion.EMPTY)
        ...
        val actor: Actor = Actor.Companion.load(MyContextHolder.Companion.myContextHolder.getNow(), actorId)
        Assert.assertNotEquals("No actor for " + DemoData.Companion.demoData.conversationAuthorSecondActorOid, 0, actorId)
        val timelineUser: Timeline = MyContextHolder.Companion.myContextHolder.getNow().timelines().get(TimelineType.SENT, actor, myAccount.origin)

 ...
 */
class MyContextTestImpl internal constructor(parent: MyContextTestImpl?, context: Context?, initializer: Any?) : MyContextImpl(parent, context, initializer) {
    ...
    override fun newCreator(context: Context?, initializer: Any?): MyContext? {
        return MyContextTestImpl(null, context, initializer)
    }
    ...
    fun getAssertionData(key: String?): AssertionData? {
        return assertionData.getOrDefault(key, AssertionData.Companion.getEmpty(key))
    }
    ...
    override fun clearNotifications(timeline: Timeline) {
        ...
        NotificationEventType.Companion.validValues.forEach(Consumer { key: NotificationEventType? -> androidNotifications.remove(key) })
    }
    ...
    override fun classTag(): String? {
        return MyContextTestImpl.Companion.TAG
    }

...
object TestSuite {
    ...
    @Volatile
    private val dataAdded = false
    fun isInitializedWithData(): Boolean {
        return org.andstatus.app.context.TestSuite.initialized && org.andstatus.app.context.TestSuite.dataAdded
    }
    ...
    fun initialize(testCase: Any?): Context? {
        ...
        if (org.andstatus.app.context.TestSuite.initialized) return org.andstatus.app.context.TestSuite.context
        var creatorSet = false
        MyLog.setMinLogLevel(MyLog.VERBOSE)
        for (iter in 1..5) {
            MyLog.i(org.andstatus.app.context.TestSuite.TAG, "Initializing Test Suite, iteration=$iter")
            if (testCase == null) {
                MyLog.e(org.andstatus.app.context.TestSuite.TAG, "testCase is null.")
                throw IllegalArgumentException("testCase is null")
            }
            val instrumentation = InstrumentationRegistry.getInstrumentation()
            if (instrumentation == null) {
                MyLog.e(org.andstatus.app.context.TestSuite.TAG, "testCase.getInstrumentation() is null.")
                throw IllegalArgumentException("testCase.getInstrumentation() returned null")
            }
            org.andstatus.app.context.TestSuite.context = instrumentation.targetContext
            if (org.andstatus.app.context.TestSuite.context == null) {
                MyLog.e(org.andstatus.app.context.TestSuite.TAG, "targetContext is null.")
            ...
            try {
                if (creatorSet ||  MyContextHolder.myContextHolder
                    ...
                    if ( MyContextHolder.myContextHolder.getFuture().future.isDone()) {
                        ...
                        MyLog.i(org.andstatus.app.context.TestSuite.TAG, "After starting FirstActivity $iter is initializing...")
                    }
                }
            } catch (e: IllegalStateException) {
                MyLog.i(org.andstatus.app.context.TestSuite.TAG, "Error caught, iteration=$iter", e)
            }
            DbUtils.waitMs(method, 3000)
        }
    ...
    fun clearAssertions() {
        org.andstatus.app.context.TestSuite.getMyContextForTest().assertions.clear()
    }
    ...
    fun setHttpConnectionMockClass(httpConnectionMockClass: Class<out HttpConnection?>?) {
        org.andstatus.app.context.TestSuite.getMyContextForTest().setHttpConnectionMockClass(httpConnectionMockClass)
    }

    fun setHttpConnectionMockInstance(httpConnectionMockInstance: HttpConnection?) {
        org.andstatus.app.context.TestSuite.getMyContextForTest().setHttpConnectionMockInstance(httpConnectionMockInstance)
    }

    fun onDataDeleted() {
        org.andstatus.app.context.TestSuite.dataAdded = false
    }

    private fun ensureDataAdded() {
        if (org.andstatus.app.context.TestSuite.dataAdded) return
    ...
    fun waitForListLoaded(activity: Activity?, minCount: Int): Int {
        ...
        for (ind in 0..39) {
            ...
            val itemsCountNew = if (list is ListView) (list as ListView).count else list.childCount
            MyLog.v(org.andstatus.app.context.TestSuite.TAG, "waitForListLoaded; countNew=$itemsCountNew, prev=$itemsCount, min=$minCount")
            if (itemsCountNew >= minCount && itemsCount == itemsCountNew) {
                break
            }
            itemsCount = itemsCountNew
        }
        Assert.assertTrue("There are " + itemsCount + " items (min=" + minCount + ")" +

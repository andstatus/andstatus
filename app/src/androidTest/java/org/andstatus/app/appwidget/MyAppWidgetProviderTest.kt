 ...
 */
package org.andstatus.app.appwidgetimport
...
class MyAppWidgetProviderTest {
    ...
    private class DateTest internal constructor(startMillis: Long, endMillis: Long) {
        var date1: Time?
        var date2: Time?
        var flags: Int
    ...
    private fun checkEvents(numMentions: Long, numPrivate: Long, numReblogs: Long) {
        val events = myContext.getNotifier().events
    ...
    private fun assertDatePeriod(message: String?, dateMin: Long, dateMax: Long, dateActual: Long) {
        ...
        if (dateActual == 0L) {
            ...
            Assert.fail(message + " actual date " + MyAppWidgetProviderTest.Companion.dateTimeFormatted(dateActual)
                    + " is less than min date " + MyAppWidgetProviderTest.Companion.dateTimeFormatted(dateMin)
                    + " min by " + (dateMin - dateActual) + " ms")
        } else {
            Assert.fail(message + " actual date " + MyAppWidgetProviderTest.Companion.dateTimeFormatted(dateActual)
                    + " is larger than max date " + MyAppWidgetProviderTest.Companion.dateTimeFormatted(dateMax)
    ...
    private fun updateWidgets(appWidgets: AppWidgets?, eventType: NotificationEventType?, increment: Int) {
        ...
        for (ind in 0 until increment) {
            NotifierTest.Companion.addNotificationEvent(appWidgets.events.myContext, eventType)
        }

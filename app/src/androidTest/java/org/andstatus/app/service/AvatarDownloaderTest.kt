...
class AvatarDownloaderTest {
    ...
    fun testDeletedFile() {
        ...
        Assert.assertTrue("Is File" + data.filename, data.file.file.isFile)
    ...
    private fun loadAndAssertStatusForMa(ma: MyAccount?, description: String?, loadStatus: DownloadStatus?,
        ...
        val commandData: CommandData = CommandData.Companion.newActorCommand(CommandEnum.GET_AVATAR, actor, actor.username)
  ...
  for $actor
  (loaded $data, error message:'${commandData.result.message}')${if (mockNetworkError) "mocked the error" else ""}"""
        ...
        Assert.assertEquals("Checking load status: $logMsg", loadStatus, loader.status)
        if (DownloadStatus.LOADED == loadStatus) {
            Assert.assertFalse("Should be no errors: $logMsg", commandData.result.hasError())
        } else {
            Assert.assertTrue("Should be an error: $logMsg", commandData.result.hasError())
        }
        Assert.assertEquals(logMsg, loadStatus, loader.status)
        if (DownloadStatus.LOADED == displayedStatus) {
            Assert.assertTrue("Avatar should be displayed: $logMsg", data.file.existed)
        } else {
            Assert.assertFalse("Avatar shouldn't be diplayed: $logMsg", data.file.existed)
        }

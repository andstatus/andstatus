...
class RepeatingFailingCommandTest : MyServiceTest() {
    ...
    fun repeatingFailingCommand() {
        for (iteration in 0 until RepeatingFailingCommandTest.Companion.ITERATIONS_NUMBER) oneIteration(iteration)
    }
    ...
    private fun oneIteration(iteration: Int) {
        ...
        AvatarDownloaderTest.Companion.changeAvatarUrl(actor, urlString)
        val startCount = mService.executionStartCount
        val endCount = mService.executionEndCount
        val requestsCounter0 = mService.http.requestsCounter
    ...
    ${mService.http}
    """.trimIndent(), mService.http.requestsCounter > requestsCounter0)
        setAndSendGetAvatarCommand(actor, false)
    ...
    private fun setAndSendGetAvatarCommand(actor: Actor?, manuallyLaunched: Boolean) {
        ...
        if (manuallyLaunched) {
            command.isManuallyLaunched = true
        }

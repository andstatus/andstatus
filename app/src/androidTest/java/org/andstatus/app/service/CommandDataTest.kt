...
class CommandDataTest {
    ...
    private fun testQueueOneCommandData(commandData: CommandData?, time0: Long) {
        ...
        Assert.assertEquals(0, commandData.getResult().lastExecutedDate)
        ...
        commandData.getResult().message = "Error in " + commandData.hashCode()
        commandData.getResult().afterExecutionEnded()
        DbUtils.waitMs(method, 50)
        val time1 = System.currentTimeMillis()
        Assert.assertTrue(commandData.getResult().lastExecutedDate >= time0)
        Assert.assertTrue(commandData.getResult().lastExecutedDate < time1)
        ...
        val commandData2 = queues[QueueType.ERROR].queue.poll()
        Assert.assertEquals(commandData, commandData2)
        // Below fields are not included in equals
        Assert.assertEquals(commandData.getCommandId(), commandData2.commandId)
        Assert.assertEquals(commandData.getCreatedDate(), commandData2.createdDate)
        Assert.assertEquals(commandData.getResult().lastExecutedDate, commandData2.result.lastExecutedDate)
        Assert.assertEquals(commandData.getResult().executionCount.toLong(), commandData2.result.executionCount.toLong())
        Assert.assertEquals(commandData.getResult().getRetriesLeft().toLong(), commandData2.result.getRetriesLeft().toLong())
        Assert.assertEquals(commandData.getResult().hasError(), commandData2.result.hasError())
        Assert.assertEquals(commandData.getResult().hasSoftError(), commandData2.result.hasSoftError())
        Assert.assertEquals(commandData.getResult().message, commandData2.result.message)
    }
    ...
    private fun assertCommand(queue: Queue<CommandData?>?, commandEnum: CommandEnum?, timelineType: TimelineType? = TimelineType.UNKNOWN) {
        val commandData = queue.poll()
        Assert.assertEquals(commandData.toString(), commandEnum, commandData.getCommand())

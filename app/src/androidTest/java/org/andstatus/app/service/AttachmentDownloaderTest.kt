...
class AttachmentDownloaderTest {
    ...
    fun testImageAttachmentLoad() {
        ...
        AttachmentDownloaderTest.Companion.loadAndAssertStatusForRow(method, dd, DownloadStatus.ABSENT, true)
        AttachmentDownloaderTest.Companion.loadAndAssertStatusForRow(method, dd, DownloadStatus.LOADED, false)
    ...
    private fun testFileProvider(downloadRowId: Long) {
        ...
        val uri: Uri = FileProvider.Companion.downloadFilenameToUri(data.file.filename)
        val `in`: InputStream = MyContextHolder.Companion.myContextHolder.getNow().context().getContentResolver().openInputStream(uri)
    ...
    companion object {
        fun loadAndAssertStatusForRow(methodExt: String?, dataIn: DownloadData?, status: DownloadStatus?, mockNetworkError: Boolean) {
            ...
            if (DownloadStatus.LOADED == status) {
                ...
                Assert.assertEquals("Status $data", status, loader.status)
            } else {
                Assert.assertTrue("Error loading " + data.uri, commandData.result.hasError())
            }
            if (DownloadStatus.LOADED == status) {
                Assert.assertTrue("File exists " + data.uri, data.file.existed)
            } else {
                Assert.assertFalse("File doesn't exist " + data.uri, data.file.existed)
            }
            Assert.assertEquals("Loaded " + data.uri, status, loader.status)
        }
    }
}
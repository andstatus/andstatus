...
class MyServiceTestHelper : MyServiceEventsListener {
    ...
    private var myContext: MyContext? = MyContextHolder.Companion.myContextHolder.getNow()
    fun setUp(accountName: String?) {
        ...
        try {
            ...
            val isSingleMockedInstance = StringUtil.isEmpty(accountName)
            if (isSingleMockedInstance) {
                httpConnectionMock = HttpConnectionMock()
                TestSuite.setHttpConnectionMockInstance(httpConnectionMock)
                MyContextHolder.Companion.myContextHolder.getBlocking().setExpired(Supplier { this.javaClass.simpleName + " setUp" })
            }
            myContext = MyContextHolder.Companion.myContextHolder.initialize(myContext.context(), this).getBlocking()
    ...
    fun tearDown() {
        ...
        MyContextHolder.Companion.myContextHolder.getBlocking().timelines().initialize()

...
class SharingMediaToThisAppTest : TimelineActivityTest<ActivityViewItem?>() {
    ...
    fun testSharingMediaToThisApp() {
        ...
        val message = ("Data was posted " + mService.getHttp().postedCounter + " times; "
                + Arrays.toString(mService.getHttp().results.toTypedArray()))
        MyLog.v(this, "$method; $message")
        Assert.assertTrue(message, mService.getHttp().postedCounter > 0)
        Assert.assertTrue(message, mService.getHttp().substring2PostedPath("statuses/update").length > 0)
